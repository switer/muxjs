...
function Ctor(options, receiveProps) {
    ...
    function _emit(propname/*, arg1, ..., argX*/) {
        arguments[0] = 'change:' + (root ? root + '.':'') + propname
    ...
    _observableKeys.forEach(function(prop) {
        _props[prop] = _valueHook(_initialProps[prop])
    ...
    /**
     *  proxy native method that has self operations
     */
    function _objectMethodsHook (propname, propvalue) {
        if (util.type(propvalue) == 'array') {
            ...
            arrayHook(propvalue, function (methodName, nativeMethod, args) {
                ...
                _props[propname] = _valueHook(propvalue)

                _emit(propname, propvalue, preValue)
     ...
     */
    function _valueHook (value) {
        switch(util.type(value)) {
            case 'object': 
                if (value instanceof Mux) props[prop] = value
                Object.defineProperty(value, '__mux__', {
                    enumerable: false,
                    value: true
                })
                return value
    ...
    function _$sync(kp, value, syncHook) {
        ...
        var nextValue = _valueHook(_props[prop])
