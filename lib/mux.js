...
function Ctor(options, receiveProps) {
    ...
    function _emitChange(propname/*, arg1, ..., argX*/) {
        ...
        var args = arguments
        var anyArgs = $util.copyArray(args)

        prefix && (prefix += '.')
        args[0] = 'change:' + prefix + propname
        _emitter.emit('change', prefix + propname)
        console.log('will trigger', propname, args[1])
        _nextTick(function () {
            console.log('trigger', propname, args[1])
            emitter.emit.apply(emitter, args)
            anyArgs[0] = prefix + propname
            anyArgs.unshift('*')
            emitter.emit.apply(emitter, anyArgs)
        })
    }
    /**
    ...
    var batches = {}
    var _pendding
    _emitter.on('change', function (keyPath) {
        batches[keyPath] = true
        if (_pendding) return
        _pendding = true
        _nextTick(function () {
            var willComputedProps = []
            var runBatches = $util.copyObject(batches)
            batches = {}
            _pendding = false

            $util.objEach(runBatches, function (k, v) {
                ;(_cptDepsMapping[k] || []).reduce(function (pv, cv) {
    ...
    function _triggerPropertyComputedChange (propname) {
        ;(_cptDepsMapping[propname] || []).forEach(function (ck/*computed key*/) {
            // values cached in meta object
            $util.patch(_cptCaches, ck, {})
            // value swap
            _cptCaches[ck].pre = _cptCaches[ck].current
            _cptCaches[ck].current = (_computedProps[ck].fn || NOOP).call(model, model)
            // emit and passing (next-value, previous-value) 
            _emitChange(ck, _cptCaches[ck].current, _cptCaches[ck].pre)
        })
    }
...
function _nextTick(fn) {
    if (!_ticks.length) _ticks.push({
        tasks: []
    })
    var curTick = _ticks[0]
    curTick.tasks.push(fn)

    if (curTick.pendding) return
    curTick.pendding = true
    setTimeout(function () {
        var runTick = _ticks.shift()
        runTick.tasks.forEach(function (f) {
            f()
        })
    }, 0)
}
