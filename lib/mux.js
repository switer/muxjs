...
function Ctor (options, receiveProps) {
    ...
    _computedKeys.forEach(function(ck) {
        ...
        defOptions[ck] = {
            enumerable: true,
            get: function() {
                return _computedMetas[ck] ? _computedMetas[ck].cached : undefined
            }
        }
    })
    ...
    function _$set(kp, value) {
        ...
        if (diff.next !== diff.pre || diff.next instanceof Object) {
            ...
            ;(_computedDepsMapping[propname] || []).forEach(function (ck/*computed key*/) {
                ...
                // value swap
                _computedMetas[propname].pre = _computedMetas[propname].next
                _computedMetas[propname].next = (_computedProps[propname].fn || NOOP).call(model)
                // emit and passing (next-value, previous-value) 
                emitter.emit('change:'+ ck, _computedMetas[propname].next, _computedMetas[propname].pre)
            })
    ...
    function _$setMulti(keyMap) {
        ...
        willComputedProps.forEach(function (ck) {
            ...
            _computedMetas[ck].pre = _computedMetas[ck].next
            _computedMetas[ck].next = (_computedProps[ck].fn || NOOP).call(model)
    ...
    Object.defineProperties(model, {
        ...
        "$get": {
            ...
            value: function(propname) {
                ...
                if else (~_computedKeys.indexOf(propname)) 
